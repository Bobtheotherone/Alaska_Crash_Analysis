...
def resolve_input_path(raw: str) -> Path:
    """
    Resolve the user's input into a concrete CSV file.

    - If raw is a file path -> use it.
    - If raw is a directory path -> try to find a .csv inside.
    """
    p = Path(raw)

    # If it's a directory, try to find a CSV in it
    if p.is_dir():
        csvs = sorted(p.glob("*.csv"))
        if not csvs:
            raise SystemExit(
                f"Error: The path {p} is a directory and contains no .csv files."
            )
        if len(csvs) > 1:
            # You can make this smarter (prompt the user) if you like
            print(
                f"Warning: The path {p} is a directory; multiple CSV files were found.",
                file=sys.stderr,
            )
            print(f"Using the first one: {csvs[0].name}", file=sys.stderr)
        return csvs[0]
...
if __name__ == "__main__":
    ...
    if args.file:
        ...
        raw = input("Enter the path to the input file or folder: ").strip().strip('"')  # use raw for input cleaning
        raw = raw.replace("'", "").replace('"', "")  # remove single quotes or double quotes if user added them
        raw_path = raw

    input_file = resolve_input_path(raw_path)

    if not input_file.exists():
        raise SystemExit(f"Error: The file {input_file} does not exist.")  # failsafe if file not found
    if not input_file.is_file():
        raise SystemExit(f"Error: The path {input_file} is not a regular file.")
    ...
    if y.nunique() < 2:
        raise SystemExit(
            "Target has fewer than 2 classes after mapping; cannot train a classifier."
        )
    ...
    # Base XGBoost model – updated for XGBoost 3.x GPU usage:
    #   tree_method='hist', device='cuda', predictor='gpu_predictor'
    baseXBG = XGBClassifier(
        n_estimators=500,
        learning_rate=0.05,
        max_depth=6,
        subsample=0.8,
        ...
        random_state=42,
        n_jobs=-1,
        tree_method="hist",       # hist algorithm (GPU-capable)
    ...
    model = Pipeline(steps=[("preprocess", preprocess), ("model", baseXBG)])
    ...
    print()
    print("Test Metrics (XGBoost Classifier):")
